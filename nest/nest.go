...
func NewUserFrontend(shards ShardsController) *userFrontend {
	return &userFrontend{shards: shards}
}
func (g *userFrontend) Serve(grpcServer *grpc.Server) {
	api.RegisterNestServer(grpcServer, g)
}

func (g *userFrontend) GetEntries(in *api.GetEntriesInput, stream api.Nest_GetEntriesServer) error {
	return errors.New("not implem yet")
}
...
func (g *shardsController) GetClusterEntries(in *api.GetClusterEntriesInput, s api.NestCluster_GetClusterEntriesServer) error {
	...
	if in.MaxCount == 0 || in.MaxCount > 20 {
		in.MaxCount = 20
	}
	...
	consumer := stream.NewConsumer(
		...
		stream.WithMaxBatchSize(int(in.MaxCount)),
	)
	...
	return consumer.Consume(s.Context(), cursor, func(_ context.Context, b stream.Batch) error {
		...
		for idx := range b.Records {
			if lastOffset != 0 && b.Records[idx].Offset() != 0 && lastOffset+1 != b.Records[idx].Offset() {
				log.Printf("%d > %d", lastOffset, b.Records[idx].Offset())
				panic("wuuut")
			}
			lastOffset = b.Records[idx].Offset()
